...
[XmlRoot(ElementName = "ENVELOPE")]
public class PostRequestEnvelope<T> : TallyXml where T : class
{
    public PostRequestEnvelope()
    {
    }

    public PostRequestEnvelope(HType Type, string iD)
    {
        Header = new(Request: RequestType.Export, Type: Type, ID: iD); //Configuring Header To get Export data
    }
    public PostRequestEnvelope(RequestType requestType, HType Type, string iD)
    {
        Header = new(Request: requestType, Type: Type, ID: iD);
    }

    public PostRequestEnvelope(HType Type, string iD, StaticVariables? sv) : this(Type, iD)
    {
