...
public class TallyDueDateJsonConverter : JsonConverter<TallyDueDate>
{
    public override TallyDueDate? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        while (reader.Read())
        {
            ...
            {
                if (billDate != null && dueDate != null & suffix != null)
                {
                    return new TallyDueDate((DateTime)billDate!, (DateTime)dueDate!, (DueDateFormat)suffix!);
                }
            ...
            if (reader.TokenType == JsonTokenType.PropertyName)
            {
                ...
                reader.Read();
                if (propertyName?.Equals("BillDate", StringComparison.InvariantCultureIgnoreCase) ?? false)
                {
