...
partial class Ledger : global::TallyConnector.Core.Models.Interfaces.ITallyRequestableObject
{
    ...
    public static global::TallyConnector.Core.Models.Request.Line GetMainTDLLine()
    {
        ...
        {
            Explode = [$"{LanguageNameList_S9R3_PartName}:YES", $"{OpeningBalance_AZIU_PartName}:YES", $"{Addresses_1S2P_PartName}:{string.Format("$$NUMITEMS:LEDMULTIADDRESSLIST>0", "Addresses")}", $"{MailingDetails_E1QZ_PartName}:{string.Format("$$NUMITEMS:LEDMAILINGDETAILS>0", "MailingDetails")}", $"{GSTRegistrationDetails_JNXL_PartName}:{string.Format("$$NUMITEMS:LEDGSTREGDETAILS>0", "GSTRegistrationDetails")}"]
        };
    }
